...
//! using the client feature, connecting to a remote VecGraph server (e.g. the server binary in this workspace).
//! Expects endpoint on localhost:50051 by default, but can be configured with the VECGRAPH_SERVER_ENDPOINT environment variable (e.g. to connect to a remote server).
//!
//! Run with:
//!   cargo run --example client --features client,embed-model2vec

use std::sync::Arc;

use vecgraph::{
    Edge, EdgeWithVector, Embedder, GraphStore, Model2VecEmbedder, Model2VecEmbedderSettings, Node,
    NodeWithVector, RemoteGraphStore, RemoteGraphStoreConfig, SearchQuery,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    pollster::block_on(async {
        let endpoint = std::env::var("VECGRAPH_SERVER_ENDPOINT")
            .unwrap_or_else(|_| "http://localhost:50051".to_string());

        // Initialise store and embedder connecting to remote server on localhost
        let store: Arc<dyn GraphStore> = Arc::new(
            RemoteGraphStore::connect(RemoteGraphStoreConfig {
                endpoint,
